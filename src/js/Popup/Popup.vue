<template>
	<Page title="Settings">
		<ServiceSelector
			:services="services"
			@serviceChanged="saveSettings"
		></ServiceSelector>
		<Grid>
			<Column width="*">
				<Button href="https://krmax44.de/donate" target="_blank"
					><Icon icon="paypal"></Icon> Donate</Button
				>
			</Column>
			<Column width="*">
				<Button href="https://github.com/krmax44/Playify" target="_blank"
					><Icon icon="github"></Icon> Contribute</Button
				>
			</Column>
		</Grid>
	</Page>
	...
	created() {
		Settings.get().then(settings => {
			let service = this.services.find(
				service => service.id === settings.service.id
			);
			service.active = true;
		});
	},
